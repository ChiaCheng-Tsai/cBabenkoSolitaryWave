// Automatically translated using m2cpp 2.0 on 2019-12-31 16:33:06

#ifndef SOLITARYGRAVITYAMPLITUDE_M_HPP
#define SOLITARYGRAVITYAMPLITUDE_M_HPP
...
void SolitaryGravityAmplitude(double Ampl, cx_vec Z, cx_vec& zs, cx_vec& ws, cx_vec& fs, vec& SWP, cx_vec& W, cx_vec& F, vec& P, cx_vec& A)
{
  cx_double S, S1, S2 ;
  cx_vec IOP, Leta_hat, Nl_hat, dW, dzsm1, eta2_hat, eta_hat ;
  double F2, Fr, L, M0, P0, a, dxi, err, etaMean, gam, kd, tol ;
  int N, n ;
  uword LZ, Niter ;
  vec COP, Ceta, Dif_err, LOI, LOP, ONES, Res_err, dexi, eta, eta1, k, phis, qs, us, vs, xi, xs ;
  if (Ampl>0.79 || Ampl<=0)
  {
    std::cerr << "The wave amplitude number must be in the interval 0 <= amplitude <= 0.79" << std::endl ;
  }
  F2 = 1+Ampl-0.05*pow(Ampl, 2)-3/70.0*pow(Ampl, 3) ;
  N = 16384 ;
  tol = 1e-15 ;
  ...
  IOP(0) = cx_double(0) ;
  eta = Ampl*arma::square((ONES/arma::cosh(0.5*kd*xi))) ;
  err = datum::inf ;
  Res_err.reset() ;
  Dif_err.reset() ;
  while (err>tol)
  {
    ...
    M0 = dxi*double(arma::as_scalar(arma::sum(eta%(1+Ceta)))) ;
    P0 = 0.5*dxi*double(arma::as_scalar(arma::sum((1+Ceta)%(arma::square(eta))))) ;
    F2 = 1+3*P0/M0 ;
    LOP = F2*COP-1 ;
    LOI = 1/LOP ;
  ...
  std::printf("| Error between two latest iterations: %5.4e |\n", err) ;
  std::printf("| Residual                           : %5.4e |\n", Res_err(Res_err.n_rows-1)) ;
  std::printf("+--------------------------------------------------+\n") ;
